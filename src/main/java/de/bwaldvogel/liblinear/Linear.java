...
public class Linear {
    ...
    public static void crossValidation(Problem prob, Parameter param, int nr_fold, double[] target, double[] scores) {
        int i;
        int l = prob.l;
        int[] perm = new int[l];
        ...
        for (i = 0; i < nr_fold; i++) {
            ...
            for (j = begin; j < end; j++) {
                target[perm[j]] = predict(submodel, prob.x[perm[j]]);
                double[] dvalues = predictValues(submodel, prob.x[perm[j]]);
                scores[perm[j]] = dvalues[0];
            }
        }
    }
